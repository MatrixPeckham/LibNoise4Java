...
import com.matrixpeckham.libnoise.module.util.Cache;
...
import com.matrixpeckham.libnoise.util.RendererNormalMap;
import com.matrixpeckham.libnoise.util.WriterTER;
import java.io.File;
...
// - A Windows bitmap file (*.png) that is colored according to elevation and
//   lit by an artificial light source.
// - A Windows bitmap file (*.png) that is colored according to elevation
//   only.
// - A Windows bitmap file (*.png) containing the normals of the terrain.  The
...
//   255.
// - A Windows bitmap file (*.png) containing the specularity map of the
...
//
// This program is free software = new free(); you can redistribute it and/or modify it
// under the terms of the GNU General Public License as published by the Free
// Software Foundation = new Software(); either version 2 of the License, or (at your option)
// any later version.
//
// This program is distributed in the hope that it will be useful, but WITHOUT
// ANY WARRANTY = new ANY(); without even the implied warranty of MERCHANTABILITY or
// FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
// (COPYING.txt) for more details.
//
// You should have received a copy of the GNU General Public License along
// with this program = new this(); if not, write to the Free Software Foundation, Inc., 59
...
//
	...
	baseContinentDef_pe0.setNoiseQuality(STD);
	...
	baseContinentDef_cu.addControlPoint(0.0000 + SEA_LEVEL, -0.375
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(0.0625 + SEA_LEVEL, 0.125
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(0.1250 + SEA_LEVEL, 0.250
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(0.2500 + SEA_LEVEL, 1.000
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(0.5000 + SEA_LEVEL, 0.250
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(0.7500 + SEA_LEVEL, 0.250
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(1.0000 + SEA_LEVEL, 0.500
		+ SEA_LEVEL);
	baseContinentDef_cu.addControlPoint(2.0000 + SEA_LEVEL, 0.500
		+ SEA_LEVEL);
	...
	baseContinentDef_pe1.setNoiseQuality(STD);
	...
	Cache<Clamp> baseContinentDef = new Cache<>(baseContinentDef_cl);
	...
	//    definition subgroup become warped = new become(); the underwater and coastal areas
	//    remain unaffected.
	...
	Cache<Select> continentDef = new Cache<>(continentDef_se);
	...
	Cache<Terrace> terrainTypeDef = new Cache<>(terrainTypeDef_te);
	...
	mountainBaseDef_rm0.setNoiseQuality(STD);
	...
	//    noise using only one octave = new one(); this information will be important in the
	//    next step.
	...
	mountainBaseDef_rm1.setNoiseQuality(BEST);
	...
	Cache<Turbulence> mountainBaseDef = new Cache<>(mountainBaseDef_tu1);
	...
	mountainousHigh_rm0.setNoiseQuality(BEST);
	...
	mountainousHigh_rm1.setNoiseQuality(BEST);
	...
	Cache<Turbulence> mountainousHigh = new Cache<>(mountainousHigh_tu);
	...
	mountainousLow_rm0.setNoiseQuality(BEST);
	...
	mountainousLow_rm1.setNoiseQuality(BEST);
	...
	Cache<Multiply> mountainousLow = new Cache<>(mountainousLow_mu);
	...
	Cache<Exponent> mountainousTerrain = new Cache<>(mountainousTerrain_ex);
	...
	hillyTerrain_bi.setNoiseQuality(BEST);
	...
	//    generates ridged-multifractal noise using only one octave = new one(); this
	...
	hillyTerrain_rm.setNoiseQuality(BEST);
	...
	//     terrain group.
	Cache<Turbulence> hillyTerrain = new Cache<>(hillyTerrain_tu1);
	...
	// really that important = new that(); they only need to "look" interesting.
	//
	// -1.0 represents the lowest elevations and +1.0 represents the highest
	// elevations.
	//
	...
	plainsTerrain_bi0.setNoiseQuality(BEST);
	...
	plainsTerrain_bi1.setNoiseQuality(BEST);
	...
	//    terrain group.
	Cache<ScaleBias> plainsTerrain = new Cache<>(plainsTerrain_sb2);
	...
	badlandsSand_rm.setNoiseQuality(BEST);
	...
	//    algorithm, small polygonal pits are generated = new are(); the edges of the pits
	...
	//    detail module.
	Cache<Add> badlandsSand = new Cache<>(badlandsSand_ad);
	...
	badlandsCliffs_pe.setNoiseQuality(STD);
	...
	//    cliffs module.
	Cache<Turbulence> badlandsCliffs = new Cache<>(badlandsCliffs_tu1);
	...
	//    terrain group.
	Cache<Max> badlandsTerrain = new Cache<>(badlandsTerrain_ma);
	...
	riverPositions_rm0.setNoiseQuality(BEST);
	...
	riverPositions_rm1.setNoiseQuality(BEST);
	...
	//    positions group.
	Cache<Turbulence> riverPositions = new Cache<>(riverPositions_tu);
	...
	scaledMountainousTerrain_pe.setNoiseQuality(STD);
	...
	scaledMountainousTerrain_ex.setSourceModule(0,
	...
	Cache<Multiply> scaledMountainousTerrain
		= new Cache<>(scaledMountainousTerrain_mu);
	...
	scaledHillyTerrain_pe.setNoiseQuality(STD);
	...
	Cache<Multiply> scaledHillyTerrain = new Cache<>(scaledHillyTerrain_mu);
	...
	Cache<ScaleBias> scaledPlainsTerrain = new Cache<>(
	...
	Cache<ScaleBias> scaledBadlandsTerrain = new Cache<>(
	...
	continentalShelf_rm.setNoiseQuality(BEST);
	...
	//    and-trenches module.
	Cache<Add> continentalShelf = new Cache<>(continentalShelf_ad);
	...
	Cache<Select> baseContinentElev = new Cache<>(baseContinentElev_se);
	...
	Cache<Add> continentsWithPlains = new Cache<>(continentsWithPlains_ad);
	...
	continentsWithHills_se.setBounds(1.0 - HILLS_AMOUNT, 1001.0
		- HILLS_AMOUNT);
	...
	Cache<Select> continentsWithHills = new Cache<>(continentsWithHills_se);
	...
	continentsWithMountains_cu.addControlPoint(1.0 - MOUNTAINS_AMOUNT,
		0.0625);
	...
	continentsWithMountains_ad1.setSourceModule(0,
		continentsWithMountains_ad0);
	continentsWithMountains_ad1.setSourceModule(1,
	...
	continentsWithMountains_se.setSourceModule(1,
	...
	Cache<Select> continentsWithMountains = new Cache<>(
	...
	continentsWithBadlands_pe.setNoiseQuality(STD);
	...
	Cache<Max> continentsWithBadlands = new Cache<>(
	...
	//    elevation units and is negative = new is(); this is required for step 2.
	...
	continentsWithRivers_se.setEdgeFalloff(CONTINENT_HEIGHT_SCALE
		- SEA_LEVEL);

	// 4: [Continents-with-rivers subgroup]: Caches the output value from the
	//    blended-rivers-to-continents module.
	Cache<Select> continentsWithRivers
		= new Cache<>(continentsWithRivers_se);
	...
	Cache<Cache<Select>> unscaledFinalPlanet = new Cache<>(
	...
	Cache<ScaleBias> finalPlanet = new Cache<>(finalPlanet_sb);
	...
	String fname = "terrain-" + GRID_HEIGHT + "x" + GRID_WIDTH + "";
	...
	if (resInMeters <= 240.0) {
	    ...
	    terrainWriter.setDestFilename(fname + ".ter");
	...
	FileOutputStream os = new FileOutputStream(new File(fname + ".raw"));

	for (y = 0; y < GRID_HEIGHT; y++) {
	    for (x = 0; x < GRID_WIDTH; x++) {
		short elev = (short) elevGrid.getValue(x, y);
		byte b = (byte) ((elev & 0xFF00) >> 8);
		byte b1 = (byte) ((elev & 0xFF00));
		os.write(b);
		os.write(b1);
	    }
	    Logger.getLogger(ComplexPlanet.class.getName()).log(Level.INFO,
		    ".raw Finished " + (y / (double) GRID_HEIGHT) * 100 + "%");
	}
	os.close();
	...
	imageRenderer.addGradientPoint(-16384.0 + seaLevelInMeters, new Color(0,
		0, 0, 255));
	imageRenderer.addGradientPoint(-256 + seaLevelInMeters, new Color(6, 58,
		127, 255));
	imageRenderer.addGradientPoint(-1.0 + seaLevelInMeters, new Color(14,
		112, 192, 255));
	imageRenderer.addGradientPoint(0.0 + seaLevelInMeters,
		new Color(70, 120, 60, 255));
	imageRenderer.addGradientPoint(1024.0 + seaLevelInMeters, new Color(110,
		140, 75, 255));
	imageRenderer.addGradientPoint(2048.0 + seaLevelInMeters, new Color(160,
		140, 111, 255));
	imageRenderer.addGradientPoint(3072.0 + seaLevelInMeters, new Color(184,
		163, 141, 255));
	imageRenderer.addGradientPoint(4096.0 + seaLevelInMeters, new Color(255,
		255, 255, 255));
	imageRenderer.addGradientPoint(6144.0 + seaLevelInMeters, new Color(128,
		255, 255, 255));
	imageRenderer.addGradientPoint(16384.0 + seaLevelInMeters, new Color(0,
		0, 255, 255));
	...
	try {
	    ...
	    ImageIO.write(destImage.asBufferedImage(), "PNG", file);
	...
	for (y = 0; y < GRID_HEIGHT; y++) {
	    for (x = 0; x < GRID_WIDTH; x++) {
		double pCur = elevGrid.getValue(x, y);
		if (pCur < (SEA_LEVEL + DEEP_SEA_LEVEL)) {
		    elevGrid.setValue(x, y, SEA_LEVEL + DEEP_SEA_LEVEL);
		}
	    }
	    Logger.getLogger(ComplexPlanet.class.getName()).log(Level.INFO,
		    "Flatten pass Finished " + (y / (double) GRID_HEIGHT) * 100
		    + "%");
	}
	...
	surfaceRenderer.addGradientPoint(-16384.0 + seaLevelInMeters, new Color(
		3, 29, 63, 255));
	surfaceRenderer.addGradientPoint(DEEP_SEA_LEVEL + seaLevelInMeters,
		new Color(3, 29, 63, 255));
	surfaceRenderer.addGradientPoint(-1.0 + seaLevelInMeters, new Color(7,
		106, 127, 255));
	surfaceRenderer.addGradientPoint(0.0 + seaLevelInMeters, new Color(62,
		86, 30, 255));
	surfaceRenderer.addGradientPoint(1024.0 + seaLevelInMeters,
		new Color(84, 96, 50, 255));
	surfaceRenderer.addGradientPoint(2048.0 + seaLevelInMeters, new Color(
		130, 127, 97, 255));
	surfaceRenderer.addGradientPoint(3072.0 + seaLevelInMeters, new Color(
		184, 163, 141, 255));
	surfaceRenderer.addGradientPoint(4096.0 + seaLevelInMeters, new Color(
		255, 255, 255, 255));
	surfaceRenderer.addGradientPoint(6144.0 + seaLevelInMeters, new Color(
		128, 255, 255, 255));
	surfaceRenderer.addGradientPoint(16384.0 + seaLevelInMeters,
		new Color(0, 0, 255, 255));
	surfaceRenderer.enableLight(false);
	surfaceRenderer.render();

	try {
	    File file = new File(fname + "-surface.png");
	...
	specularityRenderer.addGradientPoint(MIN_ELEV, new Color(255, 255, 255,
		255));
	specularityRenderer.addGradientPoint(seaLevelInMeters, new Color(255,
		255, 255, 255));
	specularityRenderer.addGradientPoint(seaLevelInMeters + 1.0,
		new Color(0, 0, 0, 255));
	specularityRenderer.addGradientPoint(MAX_ELEV, new Color(128, 128, 128,
		255));
	specularityRenderer.enableLight(false);
	specularityRenderer.render();

	try {
	    File file = new File(fname + "-spec.png");
	...
	try {
	    File file = new File(fname + "-normal.png");
